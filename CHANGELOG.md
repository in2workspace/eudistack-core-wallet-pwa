...
- **EUD-104 — "Mis dispositivos" didn't show which sessions were actually active, and logging out on one device left it looking active anyway**: `devices.page.html` already received a correct `activeSessions` count per device, but never rendered it as such — the only thing that field drove was whether to show the "close sessions" button — so two devices signed in at the same time had no visual confirmation that both were active. Each device card (current and other) now shows an explicit "Active" / "No active session" badge driven by `passkey.activeSessions`.
  - Separately, the "Cerrar sesión" menu action never told the backend anything: `RemoteAuthService.logout()` only cleared local state, so a device's refresh token stayed valid — and its session kept counting as active — even after the holder explicitly logged out and closed the browser. `logout()` now also calls `POST /api/v1/auth/logout` (now scoped to just this device server-side) before clearing local state; a failed call still logs the device out locally, it just leaves the session live in the backend until it naturally expires.
