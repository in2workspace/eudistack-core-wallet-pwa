...
- **Wallet sessions could be silently closed with no warning, sometimes less than 20 minutes after login (E-02)**: when the automatic background token refresh failed, `RemoteAuthService` redirected straight to the login screen with no explanation. It now shows the existing "your session has expired" notice in that case too. A redundant duplicate `forceLogout()` call on the same failure was also removed.
  ...
  - Added a "your session is about to expire, continue?" prompt 2 minutes before the silent background refresh point, so an active user gets a chance to extend the session with one tap instead of only finding out after the fact. Confirming refreshes immediately and resets the whole cycle; leaving it unanswered changes nothing — the existing silent refresh (and, on failure, the session-expired notice) still runs exactly as before. The prompt shows a shrinking progress bar for the 2 minutes, matching the countdown already used on the Verifier's own login page for QR session expiry.
