...
- **A device already linked to the account was sent back to email + OTP after its second logout, and "randomly" ~14 minutes after a passkey resume**: `RemoteAuthService.refreshAccessToken()` cached its in-flight request (`shareReplay({ refCount: true })`) and only cleared that cache from a `complete`/`error` hook on the observable it returned. `unlockWithPasskey()` and `ensureAccessToken()` consume it through `firstValueFrom()`, which unsubscribes right after the first value, so the hook never ran: the next refresh (the second passkey resume, or the scheduled refresh before the access token expires) re-subscribed to the stale request and re-sent the refresh token it had **already rotated**. `eudistack-core-wallet-ebw` correctly treats that as token reuse (`401 token_compromised`) and revokes every session of the device, and `clearState()` then dropped `wallet_refresh_token` — the only credential that lets a device resume with its passkey — so `LoginPage` fell back to the email step. The cache is now cleared with `finalize()` when the request itself settles, and `refCount: false` keeps a caller that leaves early from cancelling a rotation the server may already have applied. Related hardening:
  - The refresh token is only dropped when the server rejects it (`isRefreshTokenRejected`: HTTP 400/401/403). A transient failure (offline, 5xx, 429) keeps it: the background/interceptor path does a soft logout back to the passkey screen, and `LoginPage.verifyPasskey()` stays on the passkey step with `errors.network-error` instead of asking for a new OTP. `authInterceptor` no longer forces a hard logout after a refresh that already ended the session.
  - The refresh token is read from `localStorage` (source of truth) before each refresh, so an instance never presents a token another context already rotated.
  - Tests: logout + re-entry with the local passkey three times in a row, scheduled refresh after a passkey resume, token rotated by another context, concurrent refresh coalescing, transient vs rejected refresh failures (`auth.service.spec.ts`, `auth.interceptor.spec.ts`, `login.page.spec.ts`).
- **Reopening the wallet after a version update could show the "install this app" screen again on an already-installed PWA**: `PwaInstallService.installDecision$` already treats `isStandalone` as authoritative, but that check runs again from scratch on the full-page reload `SwUpdateService` triggers once a new version activates (`VERSION_READY`) — a moment where display-mode detection is more likely to misread transiently, notably on macOS Safari "Add to Dock". `SwUpdateService` now marks the session as standalone (`PwaInstallService.markPreUpdateReload()`) right before that reload, and `PwaInstallService` trusts that one-shot marker on the very next boot alongside `isStandalone`/`navigator.standalone`, so an update never forces a reinstall prompt on a session that was already running as an installed app. `manifest.webmanifest` is now also part of the Angular Service Worker's precached "app" asset group, closing a related gap where it could be served stale across a version boundary.
  ...
  - `manifest.webmanifest` now also declares an explicit `id`, so the browser has a stable identity to match an already-installed PWA against — without it, a plain browser tab could fail to recognize an existing install and offer the install screen again even though the app was already installed elsewhere.
...
- **Robust credential matching via base64url normalization**: introduced a comparison utility that accounts for encoding variations (such as padding or different character sets) in base64url-encoded credential IDs, preventing authentication failures and duplicate device registrations caused by strictly binary mismatches of semantically identical identifiers.
...
- **EUD-38 — CycloneDX inventory and license gate**: the repository generates its CycloneDX 1.6 inventory on every build, publishes it as an asset for each release (`sbom-v<version>.cdx.json`, verifying that the inventory version matches the artifact version) and evaluates each pull request against the list of allowed licenses (`.github/license-policy.json`, transcription of `conv-quality-security-gates.md` §16.1). The evaluator and its test suite live in `.github/scripts/`, without third-party dependencies and without depending on any other repository. Operating guide: `docs/_shared/guides/license-gate-and-sbom.md` in `eudistack-platform-dev`.
- **CI — Software Composition Analysis (SCA)**
  - **Trivy** added to `pr.yml`: `fs` scan of the repository root (reads `package-lock.json`), `HIGH,CRITICAL` severity, with vulnerability database cache, JSON report as an artifact and `config/trivy/.trivyignore` for documented accepted risks. Trivy's npm parser omits development dependencies by default, so the scan reflects what reaches the browser.
  - **`exit-code: 0` deliberately**: enters as a baseline, not yet as a gate. The production dependency tree drags HIGH advisories in `@angular/common`, `@angular/core` and `@angular/compiler`. A patch bump to `19.2.25` (last published 19.x) closes CVE-2026-50170 and CVE-2026-50171; the remaining six (CVE-2026-54266, 54267, 54268, 68945 and 69151) only have a fix in `20.3.25+`, `21.2.17+` or `22.x`, so cleaning the tree requires an Angular major upgrade. It will turn to `1` when that upgrade lands or when the accepted risks are listed in `.trivyignore`.
  - **CycloneDX SBOM** (`@cyclonedx/cyclonedx-npm@6.0.1`, spec 1.6, production dependencies only) generated in `ci-cd.yml`, published as an artifact with 90 days retention and attached to the GitHub Release as `sbom.cdx.json`.
  - **Dependabot**: update configuration for `npm` and `github-actions`. Having alerts activated is not enough — without a configuration file, the dependency graph was not producing alerts in this repository.
  - `config/trivy/**` added to `paths-ignore` in `ci-cd.yml`: changing the exclusion list is a policy change, not a deployment.
...
- **Hotfix — wallet degraded to browser mode on server tenants**: `HttpErrorInterceptor` resolved `AuthService` in a field initializer, and Angular builds the DI interceptor chain lazily on the first HTTP request — the wallet-discovery call fired from `APP_INITIALIZER`. `AUTH_SERVICE_PROVIDER` therefore ran `WalletDiscoveryService.mode()` before the snapshot existed, fell back to `environment.wallet_mode`, and pinned the root `AuthService` to `LocalAuthService` for the whole session: every own-backend request went out without `Authorization` and the resulting 401 forced a logout. Deterministic on every load, but only visible where the `WALLET_MODE` fallback disagreed with the tenant's real mode (dev: `browser` vs a `server` tenant). The interceptor now injects `Injector` and resolves `AuthService` at the point of use, and returns early on wallet-discovery errors so a failing well-known cannot resolve it either. Two regression tests cover both entry points.

- **A 429 while sending/resending the OTP code showed a duplicate generic error modal on top of the inline banner, and "Enviar código" stayed clickable with no cooldown**: `HttpErrorInterceptor` silenced toasts for auth endpoints via `pathname.startsWith('/api/v1/auth/')`, but `UrlResolverService.serverUrl()` falls back to `${origin}/business-wallet` in every real deployment (nginx routes the wallet's own backend under that prefix), so the actual request path is `/business-wallet/api/v1/auth/register` — a `startsWith` check never matched it outside unit tests, so the generic modal (`ToastServiceHandler.showErrorAlert`) fired alongside `LoginPage`'s own inline banner on every rate-limited send/resend. Changed to `pathname.includes('/api/v1/auth/')`, robust to any backend prefix. Separately, `sendCode()`/`resendCode()` cleared `loading` on a 429 with no cooldown, so the button was immediately clickable again despite the backend still rejecting the request — they now read the backend's `Retry-After` header (`RateLimitWebFilter`, `eudistack-core-wallet-ebw`) and start the existing resend-cooldown timer with that exact value (falling back to the default 180s if the header is ever missing), disabling "Enviar código" and showing a live "Reintentar en mm:ss" countdown until it can be clicked again. `resendCountdown()` now also formats hours, since a Retry-After cooldown can run well past the few minutes the format was originally sized for.

- **Inconsistent WebAuthn authenticator picker between Chrome and Edge on Windows during passkey login/registration**: with no explicit `hints` on `navigator.credentials.create()`/`.get()`, Chrome's account chooser surfaced every option (this device, phone/tablet via hybrid/QR, roaming security key) while Edge defaulted straight to the Windows Hello PIN prompt and never offered cross-device sign-in — the two browsers were reading different, undocumented heuristics off the same request. `hints` is a *descending preference order* (WebAuthn L3), not a filter, so a single shared array was wrong for every call site: `WEBAUTHN_CREATE_HINTS = ['hybrid', 'security-key', 'client-device']` is used on registration (`create()`, no `authenticatorAttachment`) to nudge Edge away from jumping straight to Windows Hello, while `WEBAUTHN_ASSERTION_HINTS = ['client-device']` is used on every assertion of an already-known local credential (`get()` with `allowCredentials`) — `LoginPage.authenticateLocally`, `PasskeyPrfService.evaluatePrf` (master key derivation), and `PrfClientService.detectPrfSupport`/`evaluateForWrap` (hybrid-keymanager) — so Chrome doesn't offer a phone/QR picker for a credential that lives on this device. Since `hints` is only advisory and Edge's native picker still isn't fully under the app's control, `LoginPage` also shows a dedicated hint (`auth.passkey.edge-windows-hint`, es/en/ca) on the exact Edge+Windows combination — but only on the registration screens (browser-mode create, device passkey setup), never on the verify-passkey/login screen, where the credential is already local and "open Chrome to use your phone" would be actively wrong advice. Tests: `passkey-prf.service.spec.ts` (`create` gets the hybrid-first hints, `get` gets the assertion-only hints), `login.page.spec.ts` (login `get()` never sends `hybrid` first; the Edge/Windows hint text is shown only for Edge+Windows on registration and never on the verify-passkey screen, even on that exact combination).
...
- **Support link hardcoded to the DOME marketplace ticketing site, and shown as raw unrendered text on the credentials load-error screen**: `ToastServiceHandler`'s "contact support" link was hardcoded to `https://ticketing.dome-marketplace.eu/` regardless of tenant, instead of reusing the tenant-aware resolution already in place for the About section (`SupportChannelService`, EUD-135/AD-4) — moved to `core/services/` so both features can depend on it. `SupportChannels` gained a `supportUrl` field (the theme's existing `content.supportUrl`, previously declared but unused — see `content.issueTrackerUrl`, a different, unrelated field pointing at the wallet's own GitHub bug tracker) so each tenant keeps its own support destination (DOME's ticketing site for the `dome` tenant, `docs.eudistack.net` elsewhere) without it being hardcoded in `ToastServiceHandler`. Separately, the credentials list's load-error state bound the translated message straight to `[innerHTML]`, bypassing the sanitization/link-substitution `ToastServiceHandler` already does for alerts — holders saw the literal `{{supportLink}}` placeholder instead of a clickable link. Extracted that logic into a reusable `supportLink` pipe so both paths share the same sanitization.
...
- **The "copy token" button on machine credentials disappeared in the UI redesign**: the credential detail drawer still renders the encoded token (`vc-fields.credentialEncoded`, added only for `LEARCredentialMachine` and `gx:LabelCredential`), but the redesign dropped the copy control that sat next to it, leaving a truncated, unselectable string as the only way to get at the token. `copyToClipboard()` had survived untouched in the component — only the template markup was missing, so it was re-added against the new `field-grid` layout (value box + button sharing the same border and surface tokens as the surrounding fields) rather than restoring the old `ion-button`. New `vc-fields.copy` key for the button's `aria-label`, in the three locales.
  - Copy confirmation moved from `showToast()` to `showInfoToastByTranslateLabel()`: the former builds an `ion-alert`, i.e. a centred modal with a backdrop that blocks interaction until it auto-dismisses — disproportionate for acknowledging a clipboard write. The latter is the existing non-blocking floating pill (already used by the `unknown` verification notice), here at 2s instead of its 5s default.
...
- **Passkey resume did not restore the in-memory access JWT before server calls**: after a reload the refresh token survived in `localStorage` but `LoginPage.verifyPasskey()` only exchanged it when a private flag was set, so the credentials list (IndexedDB) looked logged-in while DELETE/generate went out without `Authorization` and the interceptor treated the 401 as session expiry. `RemoteAuthService.unlockWithPasskey()` now performs local WebAuthn and, if a refresh token is present and there is no access token, calls `refreshAccessToken({ onAuthFailure: 'clear-only' })`. The auth interceptor retries own-backend 401s once via `refreshAccessToken()` before `forceLogout()`. Server-mode login does not navigate until `getToken()` is non-empty. Browser mode is unchanged (`getToken()` stays empty).
...
- **`package-lock.json` referenced `bytes@3.7.2`, a version that does not exist in the registry**: `npm ls` marked it as invalid and, with it, no component inventory could be generated. Re-resolved to `bytes@3.1.2`, the last published version and the one requested by `body-parser`, `compression` and `raw-body`.
...
- **EUD-142 US-07 — runtime UI translation to languages not natively supported** (FR-24, FR-25, FR-26): opt-in, reversible layer over the native language (es/en/ca, US-02) that translates the app's static interface text via the browser's on-device Translator API (Chrome 138+/Edge 148+ desktop) — never a cloud endpoint, never the credential screen. New "Automatic translation" section in Configuration → Language: toggle (off by default, hidden entirely when the device has no on-device engine), target-language selector, progress indicator during first preparation, and unavailable/error states with retry.
...
- **EUD-140 US-05 — Exportar el historial de actividad a CSV**: nuevo `ActivityExportService` (`providedIn: 'root'`) con un serializador CSV puro y sin dependencias — `buildCsv(entries, labels)` mapea un allow-list explÃ­cito de 5 columnas (`type` localizado, `credencial`, `contraparte`, `timestamp` en ISO 8601, `detalle`), excluyendo el `id` interno y cualquier otro campo del modelo (minimizaciÃ³n, AC-04), sin reordenar columnas ni filas (AD-2/AD-4). Incluye escapado RFC 4180 (comillas dobladas para `,`/`"`/`\r`/`\n`) + BOM UTF-8 + separador `\r\n` (EC-01), neutralizaciÃ³n de inyecciÃ³n de fÃ³rmulas anteponiendo `'` a valores de `credentialName`/`counterparty`/`details` que empiecen por `=`, `+`, `-`, `@`, TAB o CR (EC-02), y serializaciÃ³n defensiva ante campos vacÃ­os/`type` desconocido o entradas malformadas, sin lanzar excepciÃ³n ni abortar el resto de filas (EC-04, ES-01). `triggerDownload()` (`Blob` + `URL.createObjectURL` + ancla `download`, con `revokeObjectURL` en `finally`) y `buildFileName()` completan la descarga. `ActivityPage.exportHistory()` consume `entries()` (historial completo, no `filteredEntries()` — AD-1) sin mutar ninguna seÃ±al ni el filtro activo (AC-05, AC-06), y ante un fallo de descarga muestra un aviso i18n (`activity.export-error`) sin dejar archivo parcial (ES-03). Nuevo botÃ³n "Exportar historial" junto a "Borrar" en `.activity-header`, visible solo con eventos (`entries().length > 0`, ES-02). Nuevas claves i18n (`activity.export`, `activity.csv-header-*`, `activity.export-error`) en `es`/`en`/`ca`.
- **EUD-140 US-05 — test coverage**: `activity-export.service.spec.ts` (nuevo, 30 tests) cubre el serializador de forma aislada — estructura/cabecera/orden (AC-02), aislamiento/mapeo 1:1 (AC-03), minimizaciÃ³n sin `id` (AC-04), RFC 4180 + BOM (EC-01), anti-inyecciÃ³n de fÃ³rmulas (EC-02), 200 filas en < 1 s (EC-03), campos opcionales vacÃ­os (EC-04) y eventos malformados/`type` desconocido (ES-01). `activity.page.spec.ts` extendido (+21 tests) con la integraciÃ³n de botÃ³n/descarga: invocaciÃ³n de `buildCsv`+`triggerDownload` con el historial completo (AC-01), independencia del filtro activo (AC-05), ausencia de mutaciÃ³n/llamadas a `log()`/`clear()` (AC-06), disponibilidad del botÃ³n segÃºn haya historial (ES-02), toast de error ante fallo de descarga o de serializaciÃ³n (ES-03) y export de 200 entradas mixtas sin bloquear la interacciÃ³n (EC-03, component-level).
- **EUD-139 US-04 — Ver el detalle de un evento de actividad**: `ActivityDetailComponent`, un modal Ionic read-only abierto desde `ActivityPage.openDetail(entry)` al pulsar (click o teclado, con `role="button"`/`aria-label`) cualquier `.activity-card`. Muestra credencial, contraparte (etiqueta segÃºn `issued`/`presented`, omitida si estÃ¡ ausente o el tipo no aplica), fecha absoluta y un resultado fijo "Completada". Para eventos `presented` aÃ±ade una secciÃ³n de atributos compartidos, con aviso explÃ­cito cuando no hay ninguno registrado. Sin controles de escritura — solo cerrar. Tipos de actividad desconocidos degradan a una etiqueta genÃ©rica en vez de lanzar excepciÃ³n. Abrir y cerrar el modal no recarga ni muta `entries()` ni el filtro activo de la lista.
- **EUD-139 — captura de atributos compartidos en la presentaciÃ³n OID4VP**: `Oid4vpEngineService` deriva los nombres de los claims divulgados de una presentaciÃ³n SD-JWT (`deriveSharedAttributeNames`, vÃ­a `SdJwtParserService.reconstructClaims()`), excluyendo los claims registrados (`iss`, `iat`, `exp`, `cnf`, `vct`, `_sd*`), y los adjunta al registro de actividad `'presented'` (`ActivityEntry.sharedAttributes?: string[]`, nuevo parÃ¡metro opcional en `ActivityService.log()`). Cambio aditivo y no bloqueante: un fallo de parseo o una credencial no-SD-JWT degradan a "sin atributos" en vez de interrumpir la presentaciÃ³n.
- ExtraÃ­dos `formatCounterparty`/`formatAbsoluteTime` de `ActivityPage` a `shared/utils/activity-format.util.ts`, reutilizados tanto por `ActivityPage` como por `ActivityDetailComponent`.
...
- **EUD-144 US-02 — test coverage for device revocation**: `devices.page.spec.ts` extended with 20 new tests covering the full revoke flow — revoking another device (API call, dialog content, list update, session unaffected), self-revoke (reinforced message, `clearCredentialId` â†’ `forceLogout` order, unresolved credential id fails safe), and error/edge cases (409 last-passkey message, cancelling the dialog, 5xx/timeout leaving the list and session untouched).

## [3.11.5] - 2026-07-16

### Fixed - 2026-07-16

- **OID4VP — holder key not found after page reload in browser mode**: `Oid4vciEngineService` used `crypto.randomUUID()` as the holder-key ID (introduced in 3.11.3 for EUDISTACK-645). In `PasskeyPrfKeyStorageProvider`, `isEphemeral()` matches any bare UUID and routes to `generateEphemeralKey()`, which stores the key only in an in-memory `Map` — never in IndexedDB. On page reload (or next session), `resolveKeyIdByKid()` queries IndexedDB and returns `null` â†’ OID4VP throws `"No local key found for kid=<thumbprint>"`. Fixed by prefixing the holder-key ID with `holder-` so it does not match `UUID_PATTERN` and `generatePrfDerivedKey()` persists the key record to IndexedDB as intended.

## [3.11.4] - 2026-07-15

### Fixed

- **CGCOM — VCT rename `doctorid.sd.1` â†’ `urn:es.cgcom:doctorid:1`**: updated `CredentialType`, `CredentialTypeMap`, and `VerifiableCredentialSubjectDataNormalizer` to use the canonical URN-based VCT, aligning the Wallet with the DoctorID issuer configuration and the CGCOM verifier DCQL profiles.
...
- **EUD-137 US-02 — Activity history tests**: `activity.service.spec.ts` and `activity.page.spec.ts`, covering all 13 AC/EC/ES cases (0% â†’ full coverage on both files), merged alongside EUD-138's filter test suite in the same spec file.

### Changed

- **EUD-137 US-02 — Verifier/Issuer legibility**: `formatCounterparty()` reduces URLs to hostname and truncates long `did:key` identifiers (e.g. `did:key:z6Mkâ€¦sdvktH`) instead of showing them raw. Wired into EUD-138's card subtitle (`activity.subtitle-issued`/`subtitle-presented`) so the normalized value, not the raw counterparty, is what gets interpolated into the translation.
...
- **EUDISTACK-536 US-04 — `prepareSign` contract corrected** (architecture.md Â§6.2, 2026-07-03): `vp_challenge` â†’ `payload`, the full presentation payload assembled by the OID4VP engine, matching the corrected EBW contract. A KB-JWT built from `{nonce, iat}` alone (the old contract) is invalid per RFC 9901 Â§4.1.2 (missing `aud`/`sd_hash`).
...
- **EUDISTACK-534 US-02 — `OnboardingHybridComponent`**: orchestrates init â†’ PRF ceremony â†’ key generation â†’ HKDF derivation â†’ AES-GCM wrap â†’ commit â†’ zeroize. `try/finally` guarantees private key is always zeroized; wrap key cached on success, zeroized on error. Aborts before commit if PRF ceremony fails (AC-01, AC-02, AC-08, ES-04, ES-05).
...
- Silent fallback chain: well-known failure â†’ `environment.wallet_mode` â†’ default `'browser'`
  (AC-009.4aâ€“e, AD-5). Emits `console.warn` + telemetry on every fallback.
...
- **iOS PWA install onboarding wizard — refined steps for credential offer flow (EUDI-045 US-008)**.
  ...
  6. Scan the QR from inside Wallet
     Step 6 explicitly tells the user to use Wallet's `EscÃ¡ner QR` / `QR Scan` option — not the iPhone
     native camera — because scanning from the OS camera reopens Safari and loses the offer due to the
     Safari â†” standalone storage isolation. Title and subtitle updated to frame the wizard as the
     credential activation flow rather than a generic install prompt. i18n keys renumbered `step1`â€“`step6`
...
- **Settings â†’ Wallet type badge** — the Settings page now shows a wallet mode indicator (`EUDIW` when `wallet_mode === 'browser'`, `Business Wallet` when `wallet_mode === 'server'`). Allows the user to distinguish at a glance which type of wallet they are operating in, aligned with the public documentation (`docs.eudistack.net`).
...
- **Settings â†’ Knowledge base link** — item visible cuando el `theme.json` del tenant define `content.knowledgeBaseUrl`. Todos los tenants EUDIStack ahora apuntan a `https://docs.eudistack.net` (excepto DOME, que mantiene su KB propia).

### Fixed

- **`appVersion` hardcoded to 3.0.0** — `environment{,.production}.ts` and `package.json` had been out of sync for several releases; _About_ always showed `v3.0.0` regardless of the deployed bundle. Bumped to `3.4.0` in all three files (follow-up: derive `appVersion` from `package.json` at build-time to avoid future omissions).
- **`settings.page.spec.ts`** — added a `ThemeService` stub (null snapshot) to the `TestBed`. The spec was failing in CI with `NullInjectorError: No provider for HttpClient` since commit 596f3a0 (knowledge base link), because `ThemeService` injects `HttpClient` and the spec did not provide it.

### Changed (Wallet API URLs derived from window.location — multi-tenant)

- **`env.template.js`** / **`environment.production.ts`** — `server_url` and `websocket_url` are now derived from the origin at runtime (`${window.location.origin}/business-wallet` and `ws(s)://` variant for the WebSocket). Allows the same bundle to serve all tenants (`sandbox-stg`, `kpmg-stg`, `dome-stg`, â€¦) assuming CloudFront/nginx proxies `/business-wallet/*` to the corresponding tenant's EBW (EBW will expose `/business-wallet/api/v1/...` when it goes into production).
- **`.github/workflows/deploy.yml`** — removed `WALLET_API_EXTERNAL_URL` and `WALLET_API_WEBSOCKET_EXTERNAL_URL` from the `env.js` generation step. GitHub Variables deleted in the `stg` environment.
- **`src/assets/env.js`** — commented that in local dev the explicit override to `http://localhost:8083/wallet` is maintained.
- âš ï¸ EBW not active in sandbox-stg yet; when deployed it must respond under `/business-wallet/api/*` on the same origin as the SPA per tenant.
...
- **`env.template.js`** / **`.github/workflows/deploy.yml`** — removed the build-time `OID4VCI_REDIRECT_URI` variable (GitHub Variable also deleted). The same bundle is published to all tenants (`sandbox-stg`, `kpmg-stg`, `dome-stg`, â€¦) and the `redirect_uri` is derived at runtime from `window.location.origin` via the existing fallback in `environment.production.ts`. Cause of the reported 504: the fixed host (`wallet-stg.altia.eudistack.net`) did not resolve in DNS after EUDI-094.
- **`environment.production.ts`** — documented the dynamic derivation by origin.
- **`src/assets/env.js`** — local fallback aligned with the new contract (empty string).
...
- **`theme.service.ts`** — `load()` resolves the tenant from `window.location.hostname` with `resolveTenant()` and requests `/assets/tenants/<tenant>/theme.json` (absolute URL, shared bucket served by CloudFront). Legacy `assets/tenant/*` paths within the theme are rewritten to `/assets/tenants/<tenant>/*`. Helper `isRelativeAssetPath` renamed to `isSafeAssetPath`; new `toAbsoluteAssetUrl` helper for the PWA manifest.
- **`index.html`** and **`tenant-not-found.page.html`** — default favicon is now the product one (`assets/icons/pwa-192x192.png`) instead of referencing `assets/tenant/*`. `ThemeService` dynamically injects the tenant's favicon after resolving it.
- **`ngsw-config.json`** — `/assets/tenants/**` excluded from asset prefetch; the tenant theme is cached in dataGroup with freshness (1h).
- **`.github/workflows/deploy.yml`** — removed build-time injection of tenant assets and the `tenant` input. The build is now unique and published to `s3://.../wallet/`; all STG CloudFront distributions for the environment are invalidated.
- **`.github/workflows/release.yml`** — the release triggers `deploy.yml` automatically after the tag without parameterizing the tenant (a single deploy serves all tenants).

## [3.2.0] - 2026-04-23

### Changed (EUDI-094 — theme loaded from single deploy-time asset)

- **`theme.service.ts`** — `load()` now loads a single `assets/theme.json` and removes the runtime fallback (`isKnownTenant` + `resolveTenant`). Per-tenant customization is resolved in CI: `.github/workflows/deploy.yml` copies `eudistack-platform-assets/tenants/<TENANT>/theme.json` to `assets/theme.json` and `tenants/<TENANT>/*` to `assets/tenant/` before the S3 upload. `rewriteAssetPaths()` is simplified to normalize absolute `/assets/...` routes to relative (no tenant interpolation).
- **`index.html`** and **`tenant-not-found.page.html`** — default favicon migrated from `favicon.png` to `favicon.svg`, aligned with the `eudistack-platform-assets` structure (all tenants expose `favicon.svg`).

## [3.1.2] - 2026-04-23

### Fixed (EUDI-064 post-release — env suffix in tenant resolution)

- **`tenants.constants.ts`** — `resolveTenant()` now removes environment suffixes `-stg`, `-dev`, `-pre` before the `KNOWN_TENANTS` lookup. Motivation: on STG the host is `sandbox-stg.eudistack.net` and the `isKnownTenant` guard returned `false`, redirecting the user to `/tenant-not-found`. Replicates the logic already performed by `TenantDomainWebFilter` in the backend (core-issuer) and aligns with the same fix in the Credential Manager MFE.
- **`buildFallbackUrl()`** — preserves the environment suffix of the current host when reconstructing the fallback URL. Prevents a user in STG from jumping to PROD.
- **`theme.service.ts`** — replaced ad-hoc `hostname.split('.')[0]` with `resolveTenant()`.
- **`tenant-not-found.page`** — added a logo to the screen (previously there was only text).
...
- **EUDI-013:** Rename credential type IDs: `learcredential.employee.w3c.1` â†’ `.w3c.4`, `learcredential.machine.w3c.1` â†’ `.w3c.3`
