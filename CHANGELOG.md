...
- **DOME vanity domains (`wallet.dome-marketplace-lcl.org`, `wallet.dome-marketplace-sbx.org`) were never detected as the same wallet instance as the canonical STG domain**: the single-instance guard (`SingleInstanceService`) relies on a `BroadcastChannel`, which browsers scope strictly per-origin — so two tabs open on different front-door domains that actually share the same backend/tenant (confirmed in `eudistack-platform-iac`: both vanity domains route to the same ALB as the canonical `dome.stg.eudistack.net`) never saw each other, and the "wallet already open" screen never triggered between them. New `InstanceGroupService` resolves whether the current origin belongs to a configured cross-origin group (`tenants/instance-groups.json` in `eudistack-platform-assets` — deliberately excludes local/dev-network origins), and a new `RelayTransport` in `SingleInstanceService` bridges the `BroadcastChannel` across those origins via a hidden iframe pointed at a fixed anchor URL (`instance-broker.html`). Falls back to the previous same-origin-only behavior, with a console warning, if the relay doesn't connect in time.
  - **Post-review hardening**: `instance-broker.html` no longer gates its `BROKER_READY` handshake on its own allowlist fetch — on a cold connection that serialized round-trip alone could exceed the relay's connect timeout, silently degrading every tab back to the same-origin bug this fix exists to close; the handshake now fires immediately and buffers relay traffic until the allowlist arrives. `RELAY_CONNECT_TIMEOUT_MS` raised from 400ms to 2000ms as a safety net on top of that fix, not instead of it. The broker also now scopes its allowlist to the one group whose `brokerUrl` matches its own URL, instead of unioning every group in the document. `InstanceGroupService` now applies a 3s timeout to the config fetch and tolerates a malformed (`groups`-less) response instead of throwing; `SingleInstanceService.acquireTransport()` and `AppComponent`'s `elect()` call now both fail closed to the same-origin channel / leader init on any unexpected error instead of leaving `elect()` unresolved. `NAVIGATE`/`NEW_TAB` payloads now carry an already app-relative URL stripped by the sender, so the leader no longer reinterprets a follower's raw path against its own `<base href>` — only correct today because every deploy shares the same base, not by contract. Broker HTML excluded from the CI `s3 sync`'s year-long `immutable` cache-control and pushed separately with `no-store`, and added to the service worker's `freshness` data group alongside `custom-domain.json`.
