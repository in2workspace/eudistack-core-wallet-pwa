...
- **A credential lacking the power a relying party actually required could still be offered as selectable**: DCQL queries were only ever matched against credential type/`vct`, so any credential of the right type showed up regardless of what the relying party's `claims` constraints declared — the wallet ignored that field entirely. `CredentialCacheService` now evaluates `claims` against the candidate credential (walking `path`, matching `values`, with a `"*"` segment meaning "any element of this array satisfies the rest of the path"). Claims that share the same array position are required to be satisfied by the SAME element, not by two different, unrelated ones — otherwise a credential holding two separate powers could look like it held both together. A credential query with no `claims` behaves exactly as before (type/`vct` match only), so this is fully backward compatible with relying parties that don't declare any.
