...
- **Credential selector didn't make it obvious there was more than one candidate credential**: on small viewports a single tall card could fill the whole screen, hiding that the list continued below with no visual cue — a holder without technical curiosity could assume it was the only option. Added a text counter ("Hay N credenciales válidas para presentar") shown whenever more than one credential matches the request, plus a persistent scrollbar-style indicator (rendered in `ion-content`'s `fixed` slot) that stays visible while the list scrolls, since iOS Safari never renders a native, styleable scrollbar.
...
- **QR scanner kept firing the "invalid QR" alert repeatedly while the camera stayed pointed at an unreadable code**: `zxing-scanner` decodes continuously and fires `scanSuccess` on every frame, and `BarcodeScannerComponent.onCodeResult()` forwarded each one straight through with no pause or dedupe — so `ScanPage.qrCodeEmit()`'s `unsupported` branch re-triggered the error alert on every decode, stacking `ion-alert`s until the camera was moved away. `BarcodeScannerComponent` now pauses decoding (`scanStop()`) as soon as a result comes in and ignores a repeat of the same content within a 3s window; `ScanPage` resumes scanning (`resumeScanning()`) only once the alert is dismissed. That resume was itself defeated by a second, independent bug in `ToastServiceHandler.showErrorAlertByTranslateLabel`: `map(async (translatedMessage) => { ...; await alert.onDidDismiss(); })` doesn't await the async callback — `map` just emits whatever the function *returns*, and an async function returns a pending Promise immediately — so the Observable actually completed right after the alert was created, not when it was dismissed. Replaced with `switchMap((translatedMessage) => from(this.presentErrorAlert(translatedMessage)))`, which genuinely waits for `present()` + `onDidDismiss()` before completing.
