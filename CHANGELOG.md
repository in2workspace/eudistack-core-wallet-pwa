...
- **EUD-215 — `IssuerNotificationService` sent the OID4VCI notification body with camelCase keys instead of the spec's snake_case**: OID4VCI 1.0 FINAL §11.1 names the request fields `notification_id`/`event_description`, but `notifyIssuer` posted `{ notificationId, event, eventDescription }`. Went unnoticed because the Issuer's own `NotificationRequest` had the mirror-image bug (only matched the same camelCase keys) — caught once the Issuer was fixed to expect the spec-correct field names (companion fix in `eudistack-core-issuer`, surfaced by the OIDF conformance suite's `oid4vci-1_0-issuer-happy-flow` test). Body now sends `notification_id`/`event_description`.
- **EUD-215 — `/credential` requests failed with `401 Invalid DPoP proof` after the Issuer started validating the `ath` claim**: RFC 9449 §4.2/§4.3 requires DPoP proofs presented alongside an access token (resource-server calls) to include `ath`, the access token hash. `DpopService.issueProof` never computed it — unnoticed while the Issuer's `/credential` endpoint didn't validate DPoP proofs at all. Once it started (EUD-215, `eudistack-core-issuer`), every real credential request broke. `issueProof(httpMethod, httpUri, accessToken?)` now accepts an optional access token and computes `ath = base64url(SHA-256(access_token))` when present; only the `/credential` call site passes it — PAR and `/token` correctly stay without `ath`, matching the backend.
